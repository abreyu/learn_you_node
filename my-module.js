...
module.exports = function moduleFunction(directoryName, extensionFilter, callbackFunction){
	fs.readdir(directoryName, function filter(error, fileList) {
		if(error)
			return callbackFunction(error)
		
		fileList = fileList.filter(function (file) {
            return file.indexOf(`.${extensionFilter}`)>-1;
        })	
		callbackFunction(null, fileList);
})
}

//una funzione che accetta tre argomenti, 
/*
